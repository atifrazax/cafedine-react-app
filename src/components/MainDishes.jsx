...
const MainDishes = forwardRef(({data}, ref) => {
    ...
    const [quickView, setQuickView] = useState({ isOpen: false, item: {}});
  ...
  return (
    <section className='bg-slate-100 py-20'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-6'>
                {data.map((item, index)=>(
                    <div ref={index === 0 ? ref : null} key={item?._id} className='flex flex-col items-start space-y-5 m-3 bg-white '>
                        <img src={formatImage(item?.image, 800)} alt="Recipe" loading='lazy' className='h-48 w-full object-cover' />
                        <div className='flex flex-col space-y-1 p-4'>
                            ...
                            <button key={item?._id} onClick={()=>{addToCart({_id:item?._id, name: item?.name, price: item?.salePrice > 0 ? item?.salePrice : item?.price, img: item?.image});
                                ...
                                ${isAdded === item?._id ? 'scale-100 bg-[#BF124D]! cursor-none' : 'scale-95'}`}>
                                    {isAdded === item?._id ? 'Added To Cart' : 'Add To Cart'}
                                </button>
                                <button onClick={()=>{setQuickView({isOpen: true, item: item})}} 
                                className="w-fit px-5 py-3 mt-4 text-white bg-gray-100 hover:bg-gray-300 transition duration-300">
        ...
        {/* Quick View Modal */}
        {quickView.isOpen &&
        <QuickViewModal isOpen={quickView.isOpen} 
        onClose={()=>setQuickView({isOpen: false, item: {}})} product={quickView.item}
        addToCart={addToCart} isAdded={isAdded} setIsAdded={setIsAdded} />
        }
    </section>
  )
})
