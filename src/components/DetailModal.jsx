    ...
    <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded shadow-lg w-1/2 relative">
        ...
        <div>
                ...
                <div>Payment - <span className="capitalize">{product.orderResult.paymentMethod} / {product.orderResult.status}</span></div>
                <div>Shipping - <span className="capitalize">{product.shippingAddress.lname} - {product.shippingAddress.zipCode} {product.shippingAddress.address}</span></div>
                {product.lineItems.map((item, index)=>(
                    <div key={index} className="flex gap-2 py-2 pt-6">
